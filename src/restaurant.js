...
// Para este requesito de percorrer os itens e calcular o valor, consultei o diretório https://github.com/tryber/sd-012-project-js-unit-tests/blob/andre-lorenzoni-unit-tests/src/restaurant.js

const payment = () => {
  let total = 0;
  restaurant.consumption.forEach((element) => {
    if (restaurant.fetchMenu().food[element] !== undefined) {
      total += restaurant.fetchMenu().food[element];
    }
    if (restaurant.fetchMenu().drink[element] !== undefined) {
      total += restaurant.fetchMenu().drink[element];
    }
  });
